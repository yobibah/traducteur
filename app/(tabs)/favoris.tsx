...
import { AddFavoriteUnfavorite, getAll } from "../../lib/database";
...
const Favoris = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      const setup = async () => {
        ...
        setData(data.filter((item) => item.isfavorite));
      };
      setup();
    }, []),
  );
  ...
  const handleToggleFavorite = async (item) => {
    ...
    setData((prev) =>
      newValue === 0
        ? prev.filter((d) => d.id !== item.id) // retire de la liste
        : prev.map((d) =>
            d.id === item.id ? { ...d, isfavorite: newValue } : d,
          ),
    );
  };
  ...
  return (
    <SafeAreaView className="flex-1">
      <HistoriqueHeader />
      <View className="p-4">
        <Text className="text-xl font-semibold">Favoris</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text className="text-center mt-10 text-gray-400">Aucun favori</Text>
        }
      />
